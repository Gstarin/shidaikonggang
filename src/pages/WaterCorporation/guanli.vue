<template>
    <div>
      <b-breadcrumb>
        <b-breadcrumb-item active>成本管理</b-breadcrumb-item>
      </b-breadcrumb>
      <h1 class="page-title fw-semi-bold">成本管理</h1>
      <b-row>
        <b-col>
          <Widget
              ...
              <table class="table">
                <thead>
                  <tr>
                    ...
                    <th>序号</th>
                    <th>项目名称</th>
                    <th class="hidden-sm-down">项目编号</th>
                    <th class="hidden-sm-down">备注</th>
                    <th class="hidden-sm-down">具体内容</th>
                  </tr>
                </thead>
                <tbody>
  ...
  export default {
    ...
    mounted() {
      axios.get('/api/data/guanli').then(response => {
  ...
  beforeDestroy() {
      if (this.projects.length > 0) {
          axios.post('/api/save/guanli', this.projects, {
              headers: {
    ...
    methods: {
      ...
      edit(name) {
        ...
        this.$router.push({
          path:"/app/WaterCorporation/guanli2?name=" + name,
          query: {  
    name: name  
  }  

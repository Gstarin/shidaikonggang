        ...
        <b-row>
            <b-col>
                <Widget title="<h5>项目档案</h5>" customHeader settings close>
                    <TableTemplate ref="tableTemplate" 
                        :tableData="tableData" 
                        :columns="columns" 
                        :formFields="form"
                        :storageKey="'projectArchive'" 
                        :customFilter1="filterByCategory" 
                        :customFilter2="filterBySearch"
...
export default {
    ...
    data() {
        ...
        columns: [
            ...
            label: '合同名称',
            width: 120,
            render: (row) => `<span style="text-decoration: underline; color:white">${row.htmc}</span>`
            },
    ...
    methods: {
        ...
        handleCellClick(row, column, cell, event) {
            ...
            if (column.label == "合同名称") {
                this.$router.push({
                    path: "/app/zhgl/xmdangan?htmc=" + row.htmc,
                    query: {
                        hetong: row.htmc
                    }
                })

<template>
  ...
  <el-table-column prop="id" label="序号" width="100" >
                            ...
                            <el-table-column prop="htmc" label="项目名称" >
                              <template slot-scope="scope">  
      <span @click="edit(scope.row.htmc)" style="text-decoration: underline; color:white">{{ scope.row.htmc }}</span>  
  ...
  export default {
      ...
      handleCellClick(row, column, cell, event) {  
        ...
        // console.log(row.htmc)
          ...
          if(column.label=="合同名称")
           {
            this.$router.push({
            path:"/app/tzfz/Project?htmc=" + row.htmc,
          query:{
            hetong:row.htmc
         }
           })
    ...
    submitForm() {
      ...
      this.dialogVisible = false;
        ...
        edit(id) {
            this.$router.push("/app/ProjectManagement/Project?id=" + id);      },
        parseExcel() {
            if (this.file) {
                let that = this
                that.tableData = []

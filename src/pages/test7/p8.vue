...
export default {
  ...
  methods: {
    ...
    parseExcel() {
      if (this.file) {
        ...
        reader.onload = function (e) {
          ...
          const title = [
            "name", "gendeer", "nation", "id", "birthday", "age"
          ];
          ...
          for (let i = 2; i < jsonData.length; i++) {
            ...
            for (let j = 0; j < jsonData[i].length; j++) {
              if (typeof jsonData[i][j + 1] === 'number') {
                obj[title[j]] = jsonData[i][j + 1].toFixed(2); // 保留两位小数
              } else {
    ...
    fetchDataForMonth() {
      axios.post('/api/data/p8', { iddd: this.path }, {
    ...
    save() {
      axios.post('/api/save/p8', [this.tableData, this.path], {
        headers: {

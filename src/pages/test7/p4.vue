          ...
          <div id="table">
            <el-table :data="current" style="width: 100%" id="DisburseTable" @selection-change="handleSelectionChange" @cell-dblclick="handleCellDblclick">
              ...
              <el-table-column prop="phone" label="备注">
                <template slot-scope="scope">
                  <el-input v-model="scope.row.phone" class="white-input"></el-input>
                </template>
              </el-table-column>

              <el-table-column prop="satus" label="图片">
                <template slot-scope="scope">
...
export default {
  ...
  // mounted() {
  //   axios.get('/api/data/p4').then(response => {
  ...
  // },
  // beforeDestroy() {
  //   axios.post('/api/save/p4', this.tableData, {
  //     headers: {
  ...
  methods: {
    ...
    parseExcel() {
      if (this.file) {
        ...
        reader.onload = function (e) {
          ...
          const title = [
            "name", "gendeer", "nation", "id", "birthday", "age", "img", "height", "weight", 
            "phone", "satus"
          ];
          // 解析工作表数据
    ...
    fetchDataForMonth() {
      axios.post('/api/data/p4', { iddd: this.path }, {
    ...
    save() {
      axios.post('/api/save/p4', [this.tableData, this.path], {
        headers: {
          'Content-Type': 'application/json',
        },
      })

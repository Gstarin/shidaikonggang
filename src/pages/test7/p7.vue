        ...
        <Widget title="<h5>驾驶员年审费用</h5>" customHeader settings close>
          ...
          <div id="table">
            <el-table :data="current" style="width: 100%" id="DisburseTable" @selection-change="handleSelectionChange" @cell-dblclick="handleCellDblclick">
              ...
              <el-table-column prop="name" label="驾驶员名称">
                <template slot-scope="scope">
                  <el-input v-model="scope.row.name" class="white-input"></el-input>
                </template>
              </el-table-column>

              <el-table-column prop="gendeer" label="驾驶证编号">
                <template slot-scope="scope">
                  <el-input v-model="scope.row.gendeer" class="white-input"></el-input>
                </template>
              </el-table-column>

              <el-table-column prop="nation" label="身份证号">
                <template slot-scope="scope">
                  <el-input v-model="scope.row.nation" class="white-input"></el-input>
                </template>
              </el-table-column>

              <el-table-column prop="id" label="驾驶类型">
                <template slot-scope="scope">
                  <el-input v-model="scope.row.id" class="white-input"></el-input>
                </template>
              </el-table-column>

              <el-table-column prop="birthday" label="领证日期">
                <template slot-scope="scope">
                  <el-input v-model="scope.row.birthday" class="white-input"></el-input>
                </template>
              </el-table-column>

              <el-table-column prop="age" label="年审单位">
                <template slot-scope="scope">
                  <el-input v-model="scope.row.age" class="white-input"></el-input>
                </template>
              </el-table-column>

              <el-table-column prop="img" label="年审日期">
                <template slot-scope="scope">
                  <el-input v-model="scope.row.img" class="white-input"></el-input>
                </template>
              </el-table-column>

              <el-table-column prop="height" label="审核费（元）">
                <template slot-scope="scope">
                  <el-input v-model="scope.row.height" class="white-input"></el-input>
                </template>
              </el-table-column>

              <el-table-column prop="weight" label="图片">
                <template slot-scope="scope">
                  <el-input v-model="scope.row.weight" class="white-input"></el-input>
                </template>
              </el-table-column>

              <el-table-column prop="health" label="备注">
                <template slot-scope="scope">
                  <el-input v-model="scope.row.health" class="white-input"></el-input>
                </template>
              </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      selectMonth: null, // 存储选择的月份
      path: '', // 存储生成的路径信息
    };
  },
  // mounted() {
  //   this.fetchDataForMonth(); // 默认加载当前月份的数据
  // },
  computed: {
    current() {
  ...
  methods: {
    ...
    parseExcel() {
      if (this.file) {
        ...
        reader.onload = function (e) {
          ...
          const title = [
            "name", "gendeer", "nation", "id", "birthday", "age", "img", "height", "weight", "health"
          ];
    ...
    fetchDataForMonth() {
      axios.post('/api/data/p7', { iddd: this.path }, {
    ...
    save() {
      axios.post('/api/save/p7', [this.tableData, this.path], {
        headers: {
